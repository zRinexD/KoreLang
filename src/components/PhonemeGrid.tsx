...
import {
  getPhonemesForCell,
  isCellPossible,
} from "../services/phonemeGridUtils";
...
const PhonemeGrid: React.FC<PhonemeGridWithModelsProps> = ({
  ...
  const columnLabel = (key: string) =>
    t(`phonology.${isVowels ? "backness" : "place"}.${key}`);
  const rowLabel = (key: string) =>
    t(`phonology.${isVowels ? "height" : "manner"}.${key}`);
  ...
  return (
    <Card className="flex flex-col h-full min-h-0 p-4 overflow-hidden">
      ...
      {unclassified?.items?.length && unclassified.position !== "bottom" ? (
        <div
          className="p-2 mb-2 text-sm border rounded shrink-0"
          style={{
        ...
        >
          ...
          >
            {t(unclassified.titleKey)}
          </div>
          <div className="flex flex-wrap gap-2">
            {unclassified.items.map((p, i) =>
              unclassified.renderItem ? (
                unclassified.renderItem(p, i)
              ) : (
                <span
                  key={`unclassified-${i}`}
                  className="px-2 py-1 font-serif text-lg rounded"
        ...
        >
          <thead>
            <tr>
              ...
              {columns.map((col) => (
                <th
                  key={col}
                  className="p-1 text-[10px] font-bold text-center uppercase"
                  style={{ color: "var(--text-tertiary)" }}
                >
                  {columnLabel(col)}
                </th>
          ...
          <tbody>
            {rows.map((row, rowIdx) => (
              ...
              >
                <th
                  className="py-1 pl-0 pr-1 text-[10px] font-bold text-left uppercase whitespace-nowrap"
                  style={{ color: "var(--text-tertiary)" }}
                >
                  {rowLabel(row)}
                </th>
                {columns.map((col, colIdx) => {
                  ...
                  const impossible = !isCellPossible(
                    rows[rowIdx],
                    columns[colIdx],
                    isVowels
                  );
                        ...
                        {phonemes.length > 0 && hasAnyPhoneme ? (
                          phonemes.map((p, idx) => {
                            ...
                            const phonemeInstance: PhonemeInstance =
                              (p as any).phoneme && (p as any).type
                                ? (p as unknown as PhonemeInstance)
                                : {
                                    id:
                                      (p as any).id ||
                                      `${(p as any).symbol || p}-${row}-${col}`,
                                    phoneme: (typeof p === "string"
                                      ? p
                                      : (p as any).phoneme) as PhonemeType,
                                    type: isVowels ? "vowel" : "consonant",
                                  };
                            return (
                              <div
                                key={`${row}-${col}-${idx}-${PhonemeDataService.getIPA(
                                  phonemeInstance.phoneme
                                )}`}
                                className="relative group/ph"
                              >
                                {renderPhoneme(phonemeInstance)}
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    onRemove(phonemeInstance);
                                  }}
      ...
      {legend ? (
        <div
          className="mt-2 text-[10px] text-center shrink-0"
          style={{ color: "var(--text-tertiary)" }}
        >
          {legend}
        </div>
      ...
      {unclassified?.items?.length && unclassified.position === "bottom" ? (
        <div
          className="p-2 mt-2 text-xs border rounded shrink-0"
          style={{
            backgroundColor: "var(--elevated)",
            borderColor: "var(--border)",
            color: "var(--text-primary)",
          ...
          <div className="flex flex-wrap gap-1">
            {unclassified.items.map((p, i) =>
              unclassified.renderItem ? (
                ...
                <span
                  key={`unclassified-bottom-${i}`}
                  className="px-1.5 py-0.5 font-serif text-sm rounded"
                  style={{ backgroundColor: "var(--surface)" }}
                >
                  {PhonemeDataService.getIPA(p.phoneme)}
                </span>
